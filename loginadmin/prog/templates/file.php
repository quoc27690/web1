<?php
function	template_edit($url,$func,$id,$txt_cat,$txt_ten,$txt_file,$txt_hien_thi,$txt_noi_bat,$error,$lang)
{
	$lang=$_SESSION['lang'];
?>
...
<form name="frm_edit" id="frm_edit" action="" enctype="multipart/form-data" method="post" style="margin:0px;" />
	...
	<table cellpadding="0" cellspacing="0" width="100%" class="form_chung">
        ...
        <tr>
            <td>File tài liệu  :</td>
            <td><input type="file" name="txt_file" class="inputbox" style="width:90%;" /></td>
        </tr>
        <tr class="radio">
            <td>
                Hiển thị :
...
$r2 = $db->select("goon_cat","_file = 1","order by thu_tu asc");
?>
<div class="selectbox_form_chung">
    ...
    while ($row2 = $db->fetch($r2))
    {
        ...
        $r	=	$db->select("goon_file_menu","cat = '".$row2["id"]."'","order by thu_tu asc");
        while ($row = $db->fetch($r))
        {
            echo "<option value='".$row["id"]."'";
            if ($id == $row["id"]) echo " selected ";
            echo ">";
            $r5	=	$db->select("goon_file_menu_lang","id = '".$row["id"]."' and id_lang='".$_SESSION['lang']."'");
            $row5 = $db->fetch($r5);
