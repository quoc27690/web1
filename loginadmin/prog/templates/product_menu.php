<?
function	template_edit($url,$func,$txt_cat,$cat,$txt_ten,$txt_hien_thi,$txt_noi_bat,$txt_hinh,$error,$lang)
{
	$lang=$_SESSION['lang'];
?>
	...
	<table cellpadding="0" cellspacing="0" width="100%" class="form_chung">
            ...
            <td>
                <div class="selectbox_form_chung">
                    ...
                    while ($row = $db->fetch($r))
                    {
						...
						echo ">".$row["ten"]."</option>";
                        $d = $db->select("goon_product_menu_lang","id_lang = '".$_SESSION['lang']."' and cat = '".$row["id"]."' and id <> '".$txt_cat."'","order by id");
        ...
        <tr>
            ...
            <td>
                ...
                {
                    global $db;
                    $r      = $db->select("goon_product_menu_lang","id = '".$txt_cat."' and id_lang = 1");
                    $row    = $db->fetch($r);
                    ?>
                    <input type="file" name="txt_hinh" class="inputbox" style="width:50%;float:left; margin-right:15px;" />
                    <?=$row["hinh"]!="no"?"<img src='../../uploads/product/cat_".$row['hinh']."' style='float:left' align='absmiddle' />":"<img src=\"images/false.gif\" />"?>
...
{
	global $db;
	$r	=	$db->select("goon_product_menu_lang","cat = '".$txt_cat."'");
