<?
	$a1		=	$db->select("goon_media","cat = '".$txt_cat."'");
	$tong	=	$db->num_rows($a1);
	$a2 	= 	$db->select("goon_media","cat = '".$txt_cat."' and noi_bat = 1");
	$noibat = 0;
	while ($ra2 = $db->fetch($a2))
	{
		$noibat++;
	}
	$a3 	= 	$db->select("goon_media","cat = '".$txt_cat."' and hien_thi = 0");
	$hienthi = 0;
...
<div style="padding:5px 10px 0 20px">
    <div class="quan_ly">
        <img src="images/icon_media.png" align="absmiddle"/>&nbsp;<strong style="font-size:14px">THỐNG KÊ BÀI VIẾT</strong> 
    ...
    <table border="0" cellpadding="0" cellspacing="0" width="100%">
      <tr>
        <td width="40%">
            <div class="function">
                <a href="?act=media_new&txt_cat=<?=$txt_cat?>&id_lang=<?=$_SESSION["lang"]?>"><img src="images/add_new.jpg"/> Tạo bài viết mới</a>
            </div>
        </td>
        <td width="25%">
            <form method="post">
        ...
        <td width="35%" style="text-align:right;">
            <form method="post" action="?act=media_list" class="form_tk">
	...
	<?php
        if ($func == "del")
        {
            ...
            {
                $db->delete("goon_media","id = '".$tik[$i]."'");
                $db->delete("goon_media_lang","id = '".$tik[$i]."'");
            }
            admin_load("Đã xóa các tài liệu đã chọn.","?act=media_list&txt_cat=".$txt_cat."&id_lang=".$_SESSION['lang']);
            die();
        }
    ?>
    <center>
    <form action="?act=media_list" method="post" onsubmit="return confirm('Bạn có chắc chắn không ?');">
        ...
        <table class="tb_table" border="0" cellpadding="0" cellspacing="0" width="100%">
            ...
            $r_all		=	$db->select("goon_media","cat = '".$txt_cat."'");
            ...
            {
                $r		=	$db->select("goon_media","cat = '".$txt_cat."'","order by time desc limit $min, $max");
                while ($row = $db->fetch($r))
                {
                    $count++;
                    $s = $db->select("goon_media_lang","id='".$row["id"]."' and id_lang='".$_SESSION["lang"]."'");
                    $rows = $db->fetch($s);
                ?>
                    <tr class="tb_content">
                        <td><?=$count?></td>
                        <td><?=$row["hinh"]!="no"?"<img src='../../uploads/media/hinhadmin_".$row['hinh']."' style='width:40px; height:40px;'/>":"<img src=\"images/false.gif\" />"?></td>
                        ...
                        <td><a href="?act=media_edit&id=<?=$row["id"]?>&id_lang=<?=$lang?>&txt_cat=<?=$txt_cat?>"><img src="images/sua_big.png"/></a></td>
            ...
            if (!empty($_POST['txt_search']))
                {
                    ...
                    $r	=	$db->select("goon_media_lang","ten like '%".$txt_search."%' and id_lang='".$_SESSION["lang"]."'","");
            ...
            <tr class="tb_foot">
                <td colspan="9" style="text-align:left;">
                    <?php
                        if ($pages==0) echo "";
                        for($i=1;$i<=$pages;$i++) {
                            echo "<a href='?act=media_list&txt_cat=".$txt_cat."&id_lang=".$_SESSION["lang"]."&page=$i'";
