<?
	$a1		=	$db->select("goon_cms","cat = '".$txt_cat."'");
	$tong	=	$db->num_rows($a1);
	$a2 	= 	$db->select("goon_cms","cat = '".$txt_cat."' and noi_bat = 1");
	$noibat = 0;
	while ($ra2 = $db->fetch($a2))
	{
		$noibat++;
	}
	$a3 	= 	$db->select("goon_cms","cat = '".$txt_cat."' and hien_thi = 0");
	$hienthi = 0;
...
<div style="padding:5px 10px 0 20px">
    <div class="quan_ly">
        <img src="images/icon_tin.png" align="absmiddle"/>&nbsp;<strong style="font-size:14px">THỐNG KÊ BÀI VIẾT</strong> 
    ...
    <table border="0" cellpadding="0" cellspacing="0" width="100%">
      <tr>
        <td width="40%">
            <div class="function">
                <a href="?act=cms_new&txt_cat=<?=$txt_cat?>&id_lang=<?=$_SESSION["lang"]?>"><img src="images/add_new.jpg"/> Tạo bài viết mới</a>
            </div>
        </td>
        <td width="25%">
            <form method="post">
        ...
        <td width="35%" style="text-align:right;">
            <form method="post" action="?act=cms_list" class="form_tk">
    ...
    <?php
        if ($_POST['delete'])
        {
            ...
            {
                $db->delete("goon_cms","id = '".$tik[$i]."'");
                $db->delete("goon_cms_lang","id = '".$tik[$i]."'");
            }
            admin_load("Đã xóa các Bài viết đã chọn.","?act=cms_list&txt_cat=".$txt_cat."&id_lang=".$_SESSION['lang']);
            die();
        }
		if ($_POST['sap_xep'])
        {
            ...
            {
				$db->update("goon_cms","thu_tu",$thu_tu[$i],"id = '".$id[$i]."'");
            }
			admin_load("Đã cập nhật thứ tự các Bài viết đã chọn.","?act=cms_list&txt_cat=".$txt_cat."&id_lang=".$_SESSION['lang']);
            die();
        }
    ?>
    <center>
    <form action="?act=cms_list" method="post" onsubmit="return confirm('Bạn có chắc chắn không ?');">
        ...
        <table class="tb_table" border="0" cellpadding="0" cellspacing="0" width="100%">
            ...
            $r_all		=	$db->select("goon_cms","cat = '".$txt_cat."'");
            ...
            if (empty($_POST['txt_search']))
                {
                    $r		=	$db->select("goon_cms","cat = '".$txt_cat."'","order by id limit $min, $max");
                    while ($row = $db->fetch($r))
                    {
                        $count++;
                        $s = $db->select("goon_cms_lang","id='".$row["id"]."' and id_lang='".$_SESSION["lang"]."'");
                        $rows = $db->fetch($s);
                    ?>
                        <tr class="tb_content">
                            <td>
                            ...
                            <td><?=$rows["hinh"]!="no"?"<img src='../../uploads/cms/hinhadmin_".$rows['id_lang']."_".$rows['hinh']."' style='width:40px; height:40px;'/>":"<img src=\"images/false.gif\" />"?></td>
                            ...
                            <td><a href="?act=cms_edit&id=<?=$row["id"]?>&txt_cat=<?=$txt_cat?>&id_lang=<?=$_SESSION["lang"]?>"><img src="images/sua_big.png"/></a></td>
            ...
            if (!empty($_POST['txt_search']))
                {
                    ...
                    $r	=	$db->select("goon_cms_lang","ten like '%".$txt_search."%' and id_lang='".$_SESSION["lang"]."'","");
                    ...
                    ?>
                        <tr class="tb_content">
                            ...
                            <td><?=lg_date::vn_time($row["time"])?></td>
                            <td><?=get_user($row["user"],"username")?></td>
                            <td><a href="?act=cms_edit&id=<?=$row["id"]?>&id_lang=<?=$_SESSION["lang"]?>&txt_cat=<?=$txt_cat?>"><img src="images/sua_big.png"/></a></td>
                            <td><input name="tik[]" type="checkbox" value="<?=$row["id"]?>" /></td>
                        </tr>
                    <?
                    }
            ...
            <tr class="tb_foot">
                ...
                <td colspan="7" style="text-align:left;">
                    <?php
                        if ($pages==0) echo "";
                        for($i=1;$i<=$pages;$i++) {
                            echo "<a href='?act=cms_list&txt_cat=".$txt_cat."&id_lang=".$_SESSION["lang"]."&page=$i'";
