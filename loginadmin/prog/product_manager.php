...
<?
$d	=	$db->select("goon_cat","_product = 1","");
$rowd = $db->fetch($d);
?>
<div class="function" style="margin-left:30px">
	<a href="?act=product_menu_new&cat=<?=$rowd["id"]?>&id_lang=<?=$lang?>">
...
<?php
	...
	if ($delete != 0)
	{
		$r = $db->select("goon_product_lang","cat = '".$delete."'");
		while ($row = $db->fetch($r))
		{
			$db->delete("goon_product","id = '".$row["id"]."'");
			$db->delete("goon_product_lang","id = '".$row["id"]."'");
		}
		$db->delete("goon_product_menu_lang","id = '".$delete."'");
		admin_load("Đã xóa thành công.","?act=product_list&id_lang=$lang");
	}
	if ($func == "sort")
	{
		$r	=	$db->select("goon_cat","_product=1");
		while ($row = $db->fetch($r))
		{
			$id = $row["id"];
			$db->update("goon_cat","thu_tu",$order_[$id],"id = '".$id."'");
		}
		$r	=	$db->select("goon_product_menu_lang");
		while ($row = $db->fetch($r))
		{
			$id = $row["id"];
			$db->update("goon_product_menu_lang","thu_tu",$order__[$id],"id = '".$id."'");
		}
		admin_load("Đã sắp xếp thành công.","?act=product_list&id_lang=$lang");
	}
?>
<form action="?act=product_manager" method="post">
    <input type="hidden" name="func" value="sort" />
		...
		?>
			...
			?>
                    <div class="sanpham">
                        <div class="ten_sanpham"><a href="?act=product_list&txt_cat=<?=$row2["id"]?>&id_lang=<?=$lang?>" class=" <?=($txt_cat==$row2["id"])?'active':''?>"><img src="images/icon_sp.png" align="absmiddle"/>&nbsp;&nbsp;<?=$row2["ten"]?></a></div>
                        <div class="xep_sanpham"><?=show_order("order__[".$row2["id"]."]",$db->num_rows($r2),$row2["thu_tu"],"90%",0);?></div>
                        <?
							if($xoa==1)
							{
						?>
                        <div class="sua_sanpham"><a href="?act=product_menu_edit&txt_cat=<?=$row2["id"]?>&id_lang=<?=$lang?>"><img src="images/sua.png" /></a></div>
                        <div class="xoa_sanpham"><a href="?act=product_manager&delete=<?=$row2["id"]?>&id_lang=<?=$lang?>" onclick="return confirm('Tất cả bài viết sẽ bị mất hết\nBạn có chắc chắn không ?');"><img src="images/xoa.png" /></a></div>
                        <?
							}
				...
				?>    
                    	<div class="sanpham_hang">
                            <div class="ten_sanpham_hang"><a href="?act=product_list&txt_cat=<?=$row3["id"]?>&id_lang=<?=$lang?>" class=" <?=($txt_cat==$row3["id"])?'active':''?>"><img src="images/icon_sp.png" align="absmiddle"/>&nbsp;&nbsp;<?=$row3["ten"]?></a></div>
                            <div class="xep_sanpham_hang"><?=show_order("order__[".$row3["id"]."]",$db->num_rows($r3),$row3["thu_tu"],"90%",0);?></div>
                            <?
								if($xoa==1)
								{
							?>
                            <div class="sua_sanpham_hang"><a href="?act=product_menu_edit&txt_cat=<?=$row3["id"]?>&id_lang=<?=$lang?>"><img src="images/sua.png"/></a></div>
                            <div class="xoa_sanpham_hang"><a href="?act=product_manager&delete=<?=$row3["id"]?>&id_lang=<?=$lang?>" onclick="return confirm('Tất cả bài viết sẽ bị mất hết\nBạn có chắc chắn không ?');"><img src="images/xoa.png" /></a></div>
                            <?
								}
