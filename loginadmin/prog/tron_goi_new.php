...
<?php
	...
	if ($submit)
	{
		...
		else
		{
			...
			if ($OK)
			{
				...
				$selecta = "select Max(thu_tu) AS thu_tu from goon_tron_goi where cat = '".$txt_cat."'";
				$sqla=mysql_query($selecta) or die ("khong ket noi duoc");
				$rowa=mysql_fetch_array($sqla);
				$thu_tu = $rowa["thu_tu"] + 1;
				$db->insert("goon_tron_goi","cat,hien_thi,gia,noi_bat,time,user,thu_tu,giam_gia,cap_sao","'".$txt_cat."','".($txt_hien_thi+0)."','".$txt_gia."','".($txt_noi_bat+0)."','".time()."','".$thanh_vien["id"]."','".$thu_tu."','".$txt_giam_gia."','".$txt_cap_sao."'");
				$select = "select Max(id) AS max from goon_tron_goi";
				...
				{
					$db->insert("goon_tron_goi_lang","id,id_lang,ten,chu_thich,noi_dung","'".$id."','".$value."','".$db->escape($txt_ten)."','".$txt_chu_thich."','".$txt_noi_dung."'");
				}
				if ($hinh)
				{
				...
				admin_load("Đã thêm Bài viết vào CSDL","?act=tron_goi_list&txt_cat=".($txt_cat+0)."&id_lang=".$lang);
			}
		}
	}
	else
	...
	if (!$OK)
		template_edit("?act=tron_goi_new", "new", $id , $txt_cat,$txt_ten,$txt_chu_thich,$txt_hinh,$txt_noi_dung,$txt_gia,$txt_hien_thi,$txt_noi_bat,$txt_giam_gia,$txt_cap_sao,$photos,$error,$lang)
?>
</center>
