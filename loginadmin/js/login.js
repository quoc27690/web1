$(document).ready(function () {
	...
	$('#but_login').click(function(e){				
		  if(document.formLogin.log_admin_user.value == "" || document.formLogin.log_admin_pass.value == ""){
			  alertMessage("error","Chưa nhập Tài khoản / Mật khẩu");
	 ...
	 //Hidden All  Alert Message Before
	  ...
	  function unloading() { 
	   ...
	   function thongbao() { 
			  alertMessage("error","Tài khoản / Mật khẩu không đúng");
			  $('.inner').jrumble({ x: 4,y: 0,rotation: 0 });	
			  $('.inner').trigger('startRumble');
			  setTimeout('$(".inner").trigger("stopRumble")',500);
	   }
	   function check_user() { 
			$.ajax
			 ({
				...
				dataType:"html",
				url:"kt_admin.php",
				data:"log_admin_user="+$("#username_id").val()+"&log_admin_pass="+$("#password").val(),
